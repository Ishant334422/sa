...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        ...
        help_text = (
            ...
            "*🔸 /fuk* - Trigger an attack operation.\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if last_attack_time:
        ...
        if elapsed_time < COOLDOWN_PERIOD:
            ...
            await context.bot.send_message(
                chat_id=chat_id, 
                text=f"*⏳ Please wait {remaining_time.seconds // 60} minute(s) and {remaining_time.seconds % 60} second(s) before using /fuk again.*", 
                parse_mode='Markdown'
            )
            return
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /fuk <ip> <port> <duration>*", parse_mode='Markdown')
        return
